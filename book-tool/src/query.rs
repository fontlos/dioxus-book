...
pub trait PageIndex<T: Hash + Eq> {
    fn into_page_index(self, book: &Book<T>) -> PageId;
}

impl<T: Hash + Eq> PageIndex<T> for PageId {
    fn into_page_index(self, _book: &Book<T>) -> PageId {
        self
    }
}
...
pub fn get_book_content_path(book_root: impl AsRef<Path>) -> Option<PathBuf> {
    let book_root = book_root.as_ref();
    let path = book_root.join("en");
    if path.exists() {
        return Some(path);
    }
    let path = book_root.join("src");
    if path.exists() {
        return Some(path);
    }
    None
...
impl Book<PathBuf> {
    ...
    fn populate_page(
        ...
        let mut last_heading: Option<pulldown_cmark::HeadingLevel> = None;
